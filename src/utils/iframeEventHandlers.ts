...
export const handleKeydown = (bookKey: string, event: KeyboardEvent) => {
  ...
  if (event.ctrlKey && event.key.toLowerCase() === 'f') event.preventDefault();
  window.postMessage(
    {
      type: 'iframe-keydown',
      bookKey,
...
export const handleClick = (bookKey: string, event: MouseEvent) => {
  ...
  if (now - lastClickTime < DOUBLE_CLICK_INTERVAL_THRESHOLD_MS) {
    lastClickTime = now;
    window.postMessage(
      {
  ...
  const postSingleClick = () => {
    ...
    if (footnote) {
      // footnote popup not supported in this app
      return;
    }
...
export const addLongPressListeners = (bookKey: string, doc: Document) => {
  ...
  const handleLongPress = (event: Event, target: HTMLElement) => {
    ...
    if (target.localName === 'img') {
      ...
      window.postMessage({ type: 'iframe-long-press', bookKey, elementType: 'image', src: imgTarget.src }, '*');
      ...
      if (href) {
        window.postMessage({ type: 'iframe-long-press', bookKey, elementType: 'image', src: href }, '*');
      }
    } else if (target.localName === 'table' || target.closest('table')) {
      const tableTarget = (
        target.localName === 'table' ? target : target.closest('table')
      ) as HTMLTableElement;
      window.postMessage({ type: 'iframe-long-press', bookKey, elementType: 'table', html: tableTarget.outerHTML }, '*');
    }
  };
  ...
  const startPress = (event: Event) => {
    ...
    if ('clientX' in event && 'clientY' in event) {
      ...
      if (touch) pressStartPositions.set(elementToTrack, { x: touch.clientX, y: touch.clientY });
    }

    clearTimeout(pressTimers.get(elementToTrack));
    pressTimers.set(elementToTrack, setTimeout(() => handleLongPress(event, elementToTrack), longPressDuration));
  };
  ...
  const handleMove = (event: Event) => {
    ...
    if (startPos) {
      ...
      if ('clientX' in event && 'clientY' in event) {
        ...
        if (touch) { currentX = touch.clientX; currentY = touch.clientY; }
      }
      const distance = Math.sqrt(
        (currentX - startPos.x) ** 2 + (currentY - startPos.y) ** 2,
      );
  ...
  const processElements = () => {
    ...
    doc.querySelectorAll('img, table').forEach(addListeners);
    doc.querySelectorAll('svg').forEach((svg) => {
      if (svg.querySelector('image')) addListeners(svg);
    });
  };
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    if (hasNewElements) processElements();
  });
